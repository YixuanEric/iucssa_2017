...
try {
	...
	$emaildomian = substr($email, strrpos($email, "@"));
	if($emaildomian==="@iu.edu" || $emaildomian==="@umail.iu.edu" || $emaildomian==="@indiana.edu" ){
		$email = substr($email, 0,strrpos($email, "@"))."@indiana.edu";
	}
	...
	$sql="SELECT count(*) FROM zhongqiu where email='".$email."';";
	...
	if($result[0][0]>=1){
		$sql1="UPDATE zhongqiu SET name='".$name."',cellphone='".$cellphone."', weixin='".$weixin."', timeStamp=now() WHERE email='".$email."';";
	} else {
		$sql1="INSERT INTO zhongqiu (name,email,cellphone,weixin,timeStamp)  VALUES( '".$name."', '".$email."', '".$cellphone."', '".$weixin."',now());";
	}
